...
def send_message(peer_id, text, save_for_deletion=True):
    try:
        ...
        if save_for_deletion and result and result != 0:
...
def force_delete_message(peer_id, message_id, user_id=None):
    """
    ПРИНУДИТЕЛЬНО удаляет сообщение.
    Если ID = 0 или None, пытается удалить через альтернативный метод.
    """
    if not peer_id:
        return False
    
    # Если это администратор — не удаляем
    if user_id and is_admin_or_owner(peer_id, user_id):
        print(f"👑 Администратор {user_id} — сообщение НЕ удалено.")
        sys.stdout.flush()
        return False
    
    # Если ID = 0 или None — используем альтернативный метод
    if not message_id or message_id == 0:
        print(f"⚠️ ID сообщения = {message_id}, пытаемся удалить через альтернативный метод...")
        sys.stdout.flush()
        # Пытаемся удалить последнее сообщение пользователя (если знаем peer_id и user_id)
        # Этот метод не всегда работает, но попробуем
        try:
            # Получаем последние сообщения в беседе
            ...
            for msg in items:
                if msg.get('from_id') == user_id:
                    # Нашли сообщение пользователя, пытаемся удалить
                    vk.messages.delete(
                        peer_id=peer_id,
                        message_ids=[msg['id']],
                        delete_for_all=True
                    )
                    print(f"🗑️ Альтернативно удалено сообщение {msg['id']}")
                    sys.stdout.flush()
                    return True
        except Exception as e:
            print(f"❌ Альтернативное удаление не удалось: {e}")
            sys.stdout.flush()
        return False
    ...
    try:
        ...
        print(f"🗑️ Принудительно удалено сообщение {message_id}")
        sys.stdout.flush()
        return True
    except Exception as e:
        print(f"❌ Не удалось принудительно удалить {message_id}: {e}")
        sys.stdout.flush()
        return False
...
def delete_bot_messages_with_delay(peer_id, delay=BOT_MESSAGE_DELAY):
    ...
    messages_to_delete = [msg_id for msg_id in bot_messages[peer_id] if msg_id and msg_id != 0]
    bot_messages[peer_id] = []
    
    if not messages_to_delete:
        print(f"ℹ️ Нет валидных ID для удаления")
        sys.stdout.flush()
        return
    
    def delete_after_delay():
        print(f"⏳ Ожидание {delay} секунд перед удалением {len(messages_to_delete)} сообщений...")
        sys.stdout.flush()
        time.sleep(delay)
...
def send_greeting(peer_id):
    ...
    # Очищаем список новых участников
...
def process_message(event):
    ...
    # =========================================================
    # ЕСЛИ НЕТ ССЫЛКИ — УДАЛЯЕМ СООБЩЕНИЕ (текстовое сообщение)
    # =========================================================
    if not vk_link:
    ...
    if not vip_ok:
        ...
        force_delete_message(peer_id, message_id, user_id)  # ← УДАЛЯЕМ ССЫЛКУ
    ...
    # =========================================================
    # ВСЕ УСЛОВИЯ ВЫПОЛНЕНЫ — ПУБЛИКУЕМ ССЫЛКУ (НЕ УДАЛЯЕМ)
    # =========================================================
    ...
    # Отправляем финальное сообщение (оно будет удалено через 40 секунд)
