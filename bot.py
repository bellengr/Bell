...
def force_delete_message(peer_id, message_id, user_id=None):
    ...
    if not message_id or message_id == 0:
        print(f"⚠️ ID сообщения = {message_id}, ищем в истории беседы...")
        sys.stdout.flush()
        try:
            ...
            for msg in items:
                if msg.get('from_id') == user_id:
                    ...
                    if msg_id:
                        ...
                        print(f"🗑️ Удалено сообщение {msg_id} (найдено через историю)")
                        sys.stdout.flush()
                        return True
            print(f"⚠️ Не найдено сообщение пользователя {user_id} в истории")
            sys.stdout.flush()
            return False
        except Exception as e:
...
def delete_bot_messages_with_delay(peer_id, delay=BOT_MESSAGE_DELAY):
    ...
    def delete_after_delay():
        ...
        print(f"⚠️ Нет валидных ID, ищем сообщения бота в истории...")
        sys.stdout.flush()
        try:
            history = vk.messages.getHistory(
                peer_id=peer_id,
                ...
                if msg.get('from_id', 0) < 0:
                    ...
                    if msg_id:
                        try:
                            ...
                            deleted_count += 1
                            print(f"🗑️ Удалено сообщение бота {msg_id} (найдено через историю)")
                        except Exception as e:
                            print(f"❌ Ошибка удаления {msg_id}: {e}")
                        sys.stdout.flush()
            print(f"🗑️ Итог: удалено {deleted_count} сообщений бота")
