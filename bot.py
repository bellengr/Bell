...
BOT_MESSAGE_DELAY = 15  # Задержка перед удалением сообщений бота (секунд)

queue = []
pending_links = {}
bot_messages = {}  # {peer_id: [message_id1, message_id2, ...]}
vip_links = []
...
def send_message(peer_id, text, save_for_deletion=True):
    ...
    try:
        ...
        if save_for_deletion and result:
            ...
            print(f"💾 Сохранено сообщение {result} для удаления в беседе {peer_id}")
        return result
    except Exception as e:
...
def delete_message_immediately(peer_id, message_id, user_id=None):
    """Мгновенно удаляет сообщение"""
    if not message_id or not peer_id:
        return False
    
    if user_id and is_admin_or_owner(peer_id, user_id):
        print(f"👑 Пользователь {user_id} — администратор. Сообщение НЕ удалено.")
        return False
    ...
    try:
        ...
        print(f"🗑️ Сообщение {message_id} удалено мгновенно")
        return True
    except Exception as e:
...
def delete_bot_messages_with_delay(peer_id, delay=BOT_MESSAGE_DELAY):
    """Удаляет все сообщения бота в беседе через задержку"""
    if not peer_id:
        return
    
    if peer_id not in bot_messages or not bot_messages[peer_id]:
        print(f"ℹ️ Нет сообщений бота для удаления в беседе {peer_id}")
        return
    
    messages_to_delete = bot_messages[peer_id].copy()
    
    # Очищаем список сообщений для этой беседы (чтобы не удалять дважды)
    bot_messages[peer_id] = []
    
    def delete_after_delay():
        print(f"⏳ Ожидание {delay} секунд перед удалением {len(messages_to_delete)} сообщений бота...")
        sys.stdout.flush()
        time.sleep(delay)
        ...
        for msg_id in messages_to_delete:
            try:
                ...
                if 'already deleted' in error_msg or 'not found' in error_msg or 'access' in error_msg:
                    print(f"ℹ️ Сообщение {msg_id} уже удалено или недоступно")
                else:
                    print(f"❌ Не удалось удалить {msg_id}: {e}")
            sys.stdout.flush()
        
        print(f"🗑️ Итог: удалено {deleted_count} из {len(messages_to_delete)} сообщений бота в беседе {peer_id}")
        sys.stdout.flush()
...
def handle_new_member(peer_id, user_id):
    ...
    try:
        ...
        print(f"✅ Приветствие отправлено. ID: {result}")
        sys.stdout.flush()
        
        # Удаляем приветствие через 40 секунд
        def delete_after_delay():
            time.sleep(40)
            if result:
                try:
                    ...
                    if 'already deleted' in error_msg or 'not found' in error_msg:
                        print(f"ℹ️ Приветствие уже удалено")
                    else:
                        print(f"❌ Не удалось удалить приветствие: {e}")
                sys.stdout.flush()
...
def handle_vip_commands(text, user_id, peer_id, message_id):
    ...
    if vip_match:
        ...
        if not vk_link:
            send_message(peer_id, f"❌ Не удалось распознать ссылку.", save_for_deletion=True)
            return True

        for vip in vip_links:
            if vip['link'] == vk_link:
                send_message(peer_id, f"⚠️ Ссылка {vk_link} уже есть в VIP-списке.", save_for_deletion=True)
                return True
        ...
        send_message(peer_id, f"⭐ VIP-ссылка {vk_link} добавлена! Действует до: {expires_at.strftime('%d.%m.%Y %H:%M')}", save_for_deletion=True)
        return True

    if text.lower().startswith('!delvip'):
        parts = text.split()
        if len(parts) < 2:
            send_message(peer_id, "❌ Формат: !delvip wall-123_456", save_for_deletion=True)
            return True

        if not is_group_admin(user_id):
            send_message(peer_id, "❌ Только администраторы группы могут удалять VIP-ссылки.", save_for_deletion=True)
            return True
        ...
        if not vip_to_remove:
            send_message(peer_id, f"⚠️ VIP-ссылка {vk_link} не найдена.", save_for_deletion=True)
            return True

        vip_links.remove(vip_to_remove)
        save_vip_links()
        send_message(peer_id, f"✅ VIP-ссылка {vk_link} удалена.", save_for_deletion=True)
        return True

    if text.lower() == '!vip_list':
        cleanup_vip_links()
        if not vip_links:
            send_message(peer_id, "📭 Активных VIP-ссылок нет.", save_for_deletion=True)
            return True
        ...
        send_message(peer_id, vip_text, save_for_deletion=True)
        return True
...
def process_message(event):
    ...
    if not vk_link:
        ...
        send_message(peer_id, "Для публикации нужна ссылка на контент ВКонтакте.", save_for_deletion=True)
        return

    print(f"   ✅ Ссылка найдена: {vk_link}")
    sys.stdout.flush()
    ...
    if not can_user_post(user_id):
        ...
        delete_message_immediately(peer_id, message_id, user_id)
        send_message(peer_id, f"Ты можешь отправить новую ссылку только после {need_to_wait} чужих постов. Сейчас прошло {posts_after}.", save_for_deletion=True)
        return
    ...
    # 🗑️ УДАЛЯЕМ ВСЕ СООБЩЕНИЯ БОТА ЧЕРЕЗ 15 СЕКУНД
    delete_bot_messages_with_delay(peer_id, BOT_MESSAGE_DELAY)

    # Отправляем финальное сообщение (оно тоже будет удалено через 15 секунд)
    send_message(peer_id, f"✅ Ссылка {vk_link} опубликована!", save_for_deletion=True)
