...
def send_message(peer_id, text, save_for_deletion=True):
    """Отправляет сообщение и ПРИНУДИТЕЛЬНО сохраняет его для удаления"""
    try:
        ...
        # ✅ ПРИНУДИТЕЛЬНО СОХРАНЯЕМ ДЛЯ УДАЛЕНИЯ
        if save_for_deletion and result is not None and result != 0:
            if peer_id not in bot_messages:
                bot_messages[peer_id] = []
            bot_messages[peer_id].append(result)
            print(f"💾 Сохранено сообщение {result} для удаления (всего в очереди: {len(bot_messages[peer_id])})")
            sys.stdout.flush()
        elif save_for_deletion and (result is None or result == 0):
            print(f"⚠️ Невозможно сохранить сообщение для удаления (result = {result})")
            sys.stdout.flush()
...
def delete_bot_messages_with_delay(peer_id, delay=BOT_MESSAGE_DELAY):
    ...
    # Фильтруем только реальные ID (не 0 и не None)
    messages_to_delete = []
    for msg_id in bot_messages[peer_id]:
    ...
    if not messages_to_delete:
        print(f"ℹ️ Нет валидных ID для удаления в беседе {peer_id}")
        sys.stdout.flush()
        return
...
def schedule_greeting(peer_id, user_id):
    ...
    if peer_id in greeting_timers and greeting_timers[peer_id].is_alive():
        print(f"⏳ Таймер уже запущен для беседы {peer_id}")
        return
    ...
    print(f"⏳ Запланировано приветствие для беседы {peer_id} (ждём 3 секунды)")
    sys.stdout.flush()
