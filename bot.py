...
bot_messages = {}  # Хранит сообщения бота для удаления
bot_messages_lock = threading.Lock()

# Кэш администраторов и владельцев
admin_cache = {}
...
def process_message(event):
    ...
    try:
        ...
        # Пробуем получить ID сообщения разными способами
        message_id = 0
        
        # Способ 1: conversation_message_id
        if 'conversation_message_id' in message:
            message_id = message['conversation_message_id']
            print(f"📍 ID из conversation_message_id: {message_id}")
        
        # Способ 2: id
        elif 'id' in message:
            message_id = message['id']
            print(f"📍 ID из id: {message_id}")
        
        # Способ 3: cmid
        elif 'cmid' in message:
            message_id = message['cmid']
            print(f"📍 ID из cmid: {message_id}")
        
        # Способ 4: Ищем в истории
        if message_id == 0:
            print(f"⚠️ ID сообщения = 0, ищем в истории...")
            try:
                rate_limit()
                ...
                for msg in items:
                    if msg.get('from_id') == user_id and msg.get('text', '') == text:
                        message_id = msg.get('id', 0)
                        if message_id:
                            print(f"📍 ID найден в истории: {message_id}")
                            break
            except Exception as e:
