...
# ПРИНУДИТЕЛЬНЫЙ ВЫВОД В ЛОГИ (сразу после импортов)
print("=" * 60)
print("БОТ НАЧАЛ ЗАГРУЗКУ (версия с отладкой)")
print("=" * 60)
sys.stdout.flush()
...
print(f"✅ Токен загружен: {TOKEN[:15]}... (первые 15 символов)")
...
def check_post_exists(wall_id):
    """Проверяет, существует ли пост. ВСЕ ОШИБКИ ВЫВОДЯТСЯ В ЛОГИ"""
    parts = wall_id.replace('wall', '').split('_')
    owner_id = int(parts[0])
    post_id = int(parts[1])
    
    print(f"🔍 Проверяю пост: owner_id={owner_id}, post_id={post_id}")
    sys.stdout.flush()
    
    try:
        response = vk.wall.getById(posts=f"{owner_id}_{post_id}")
        print(f"✅ Пост найден! Ответ API: {response}")
        sys.stdout.flush()
        return True
    except Exception as e:
        print(f"❌ ОШИБКА при проверке поста {wall_id}:")
...
def process_message(event):
    ...
    # ПРОВЕРКА ПОСТА С ВЫВОДОМ В ЛОГИ
    if not check_post_exists(wall_id):
        delete_message(peer_id, message_id)
        send_message(
            peer_id,
