...
# Хранилище для отслеживания новых участников
pending_new_members = {}  # {peer_id: [user_id1, user_id2, ...]}
greeting_timers = {}      # {peer_id: timer_thread}
...
def send_message(peer_id, text, save_for_deletion=True):
    try:
        ...
        if save_for_deletion and result:
            ...
            print(f"💾 Сохранено сообщение {result}")
        return result
    except Exception as e:
...
def send_greeting(peer_id):
    """Отправляет одно общее приветствие для всех новых участников"""
...
def schedule_greeting(peer_id, user_id):
    """Планирует отправку приветствия с задержкой 3 секунды"""
