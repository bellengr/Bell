...
# Типы контента, которые не поддерживают лайки
UNSUPPORTED_TYPES = ['album', 'poll', 'doc', 'app', 'page', 'event']
...
def is_admin_or_owner(peer_id: int, user_id: int) -> bool:
    ...
    return is_group_admin(user_id) or is_chat_owner(peer_id, user_id)
...
def get_content_type(vk_link: str) -> Tuple[str, int, int]:
    """
    Правильное определение типа контента для VK API likes.isLiked
    Возвращает: (тип_контента, owner_id, item_id)
    """
...
def is_supported_type(vk_link: str) -> bool:
    """Проверка, поддерживает ли тип контента лайки"""
    for unsupported in UNSUPPORTED_TYPES:
        if vk_link.startswith(unsupported):
            return False
    return True
...
def check_like(user_id: int, vk_link: str) -> bool:
    ...
    if not is_supported_type(vk_link):
        return True  # Пропускаем неподдерживаемые типы
    
    content_type, owner_id, item_id = get_content_type(vk_link)
    
    # Если тип не определен - пропускаем проверку
    if not content_type:
        return True
    ...
    try:
        ...
        response = vk.likes.isLiked(**params)
        ...
        if e.code in [15, 100, 200, 203, 204]:
            # Нет доступа или неподдерживаемый тип - пропускаем
            return True
        else:
...
def delete_message(peer_id: int, message_id: int) -> bool:
    ...
    try:
        ...
        if e.code == 15:
            ...
            print(f"❌ Нет прав на удаление сообщения {message_id}")
            return False
        else:
...
def delete_bot_messages(peer_id: int, delay: int = BOT_MESSAGE_DELAY):
    ...
    def delete_after_delay():
        ...
        print(f"🗑️ Удалено {deleted_count} из {len(messages_to_delete)} сообщений бота")
        sys.stdout.flush()
...
def process_message(event):
    ...
    if is_admin_or_owner(peer_id, user_id):
        print(f"   👑 Пользователь {user_id} - админ/владелец, сообщение не обрабатываем")
        return
        ...
        try:
            ...
            if payload and payload.get('type') == 'chat_invite_user':
                ...
                if user_id and peer_id:
                    # Отправляем приветствие новому участнику
